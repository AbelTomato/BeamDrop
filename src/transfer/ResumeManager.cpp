...
std::string unescape_json_string(const std::string &value) {
    ...
    if (escaping) {
        throw TransferError{ErrorCode::InvalidResumeState, "invalid resume state file"};
    }
    return unescaped;
}
...
std::uint64_t require_u64_field(const std::string &json, const std::string &name) {
    ...
    try {
        ...
        if (parsed_chars != match[1].str().size()) {
            throw TransferError{ErrorCode::InvalidResumeState, "invalid resume state number"};
        }
...
std::string read_text_file(const std::filesystem::path &path) {
    ...
    if (!input) {
        throw TransferError{ErrorCode::FileOpenFailed,
                            "failed to open resume state file: " + path.string()};
    }
...
bool same_identity(const ResumeRecord &record, const std::string &relative_path, std::uint64_t size,
...
void ResumeManager::save(const std::vector<ResumeRecord> &records) const {
    ...
    if (!output) {
        throw TransferError{ErrorCode::FileOpenFailed,
                            "failed to open resume state file for writing: " +
                                state_file_.string()};
    }
    ...
    for (std::size_t i = 0; i < records.size(); ++i) {
        ...
        output << "    {\"relative_path\":\"" << escape_json_string(record.relative_path)
               << "\",\"size\":" << record.size << ",\"sha256\":\""
               << escape_json_string(record.sha256) << "\",\"offset\":" << record.offset << "}";
        if (i + 1 != records.size()) {
            output << ',';
        }
        output << '\n';
    ...
    if (!output) {
        throw TransferError{ErrorCode::FileWriteFailed,
                            "failed to write resume state file: " + state_file_.string()};
    }
}
...
void ResumeManager::update_offset(const std::string &relative_path, std::uint64_t size,
                                  const std::string &sha256, std::uint64_t offset) const {
    if (relative_path.empty() || sha256.empty()) {
        throw TransferError{ErrorCode::InvalidResumeState, "invalid resume record identity"};
    }
    if (offset > size) {
        throw TransferError{ErrorCode::InvalidResumeOffset, "resume offset exceeds file size"};
    }
...
void ResumeManager::clear(const std::string &relative_path, std::uint64_t size,
                          const std::string &sha256) const {
    auto records = load();
