...
std::vector<FileEntry> scan_files(const std::filesystem::path& input_path) {
    if (!std::filesystem::exists(input_path)) {
        throw std::runtime_error("scan input path does not exist: " + input_path.string());
    }

    std::vector<FileEntry> entries;

    if (std::filesystem::is_regular_file(input_path)) {
        entries.push_back(FileEntry{input_path,
                                    to_protocol_path(input_path.filename()),
                                    std::filesystem::file_size(input_path)});
        return entries;
    }

    if (!std::filesystem::is_directory(input_path)) {
        throw std::runtime_error("scan input path is not a regular file or directory: " +
                                 input_path.string());
    }
    ...
    for (const auto& item : std::filesystem::recursive_directory_iterator(input_path)) {
        ...
        entries.push_back(FileEntry{item.path(),
                                    to_protocol_path(relative),
