...
- REST route 当前没有 EventBus 发布；实时事件、heartbeat 和 WebSocket 断线语义属于 Slice 6。
...
- FastAPI/Pydantic DTO 与真实 REST 已在 Slice 5 实现；WebSocket 传输和 FastApiAdapter 的 wire-format 解析测试仍属于 Slice 6–7。
...
- Linux：使用 Python 3.12 venv 构建 `beamdrop_native`，并通过 CTest 3/3；Python binding 测试成功 import 模块并完成 loopback、中文路径、SHA256、进度和异常映射验证。
