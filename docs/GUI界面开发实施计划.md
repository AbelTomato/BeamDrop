...
- 控制台引入 `frontend/src/components/ui/background-paths.tsx`：双向 SVG 贝塞尔轨迹以青绿、蓝紫、暖黄渐变模拟 BeamDrop 的“光束投递”路径；背景仅用于装饰，不承载状态或交互。
- 页面保留既有发送、接收、任务和错误处理流程。Header 与业务卡片使用半透明层，确保流光背景不降低表单与状态文本可读性；小屏仍沿用原有单列布局。
- 流光背景使用 `aria-hidden` 与 `pointer-events-none`，确保仅作装饰、不干扰辅助技术或页面点击；为保证传输时的页面流畅度，最终实现不使用持续动画库。

### 邻光暗色传输卡片（2026-07-15）

- 控制面采用 slate-900 暗色基底、slate-800 模块卡片、slate-700 边界与 cyan 主强调色，匹配实时局域网传输的 cyberpunk-adjacent 视觉，而不是复用室内营销站的浅色卡片。
- 发送、接收和任务模块保留原领域回调、表单校验与状态操作；卡片 hover 时提升 cyan 边框和环境光。任务项单独使用 slate 卡片、按状态区分 cyan / emerald / rose 徽标，进度条具有连续宽度过渡和 cyan 发光。

### 流光性能收敛（2026-07-15）

- 移除 `framer-motion`：原实现同时持续驱动 96 个 SVG `motion.path`、SVG blur filter 及三张卡片的 spring 动画，导致低性能设备在重绘和合成阶段出现卡顿。
- 背景收敛为 24 条静态 SVG 渐变路径，保留 BeamDrop 的光束语义；移除卡片逐帧动画、`backdrop-filter` 和大面积 CSS `blur`，仅保留短时 CSS hover transition。
- 发送、接收、任务卡片头部删除编号和描述性提示，仅保留“发送文件”“接收服务”“任务列表”标题。移除依赖后生产 JS 从 367.72 kB（gzip 117.18 kB）降至 244.55 kB（gzip 77.56 kB）。

### 字体策略（2026-07-15）

- 通过 Google Fonts 使用 `Noto Serif SC`（思源宋体）作为 CJK 字体，使用高可读性的 `Lora` 作为英文衬线字体；Latin glyph 会优先采用 Lora，中文缺字自动回退至思源宋体。
- 输入框和按钮保留思源宋体，避免路径、IP、端口和操作文案因花体影响可读性。
