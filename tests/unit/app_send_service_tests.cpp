...
void expect_service_error(const beamdrop::app::SendRequest &request,
                          beamdrop::app::ErrorCode expect_code) {
    beamdrop::app::ServiceResult<beamdrop::app::SendResult> result =
        beamdrop::app::SendService{}.send(request);
    assert(!result);
...
void test_send_request_error() {
    auto request =
        beamdrop::app::SendRequest{std::vector<std::filesystem::path>{}, "0.0.0.0", 9090};
...
void test_throw_if_cancelled() {
    ...
    auto request = beamdrop::app::SendRequest{
        std::vector<std::filesystem::path>{"./data.json"}, "0.0.0.0", 9090, 1024 * 1024, {}, token};

    source.request_stop();
    expect_service_error(request, beamdrop::app::ErrorCode::Cancelled);
}
